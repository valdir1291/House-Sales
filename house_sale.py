...
def set_feature(data): #adicionando novas colunas
...
def overview_data(data):
    ...
    st.sidebar.write("Portfolio do Projeto completo disponivel no: "
                         "[GitHub](https://github.com/valdir1291/House-Sales)")
   ...
   # Dataframe com as estatíticas descritiva
   ...
   # adicionando um botão para cada dataframe
...
def recomendacao(data,geofile):
    ...
    #dataframe compras
    df_compra = data[['id','bedrooms', 'zipcode','waterfront_is','season', 'lat','long', 'price', 'condition_type','yr_built','date']]
    ...
    #Filtro com sugestão de compra ou não compra
    ...
    #Filtrar pelo CEP da região
    f_zipcode = st.sidebar.multiselect(
    'Filtrar pelo Código Postal', df_compra['zipcode'].unique() )
    ...
    #Filtrar por imóveis com vista para água
    f_waterfront = st.sidebar.multiselect(
    'Imóveis com visão para água', df_compra['waterfront_is'].unique() )

    if (f_waterfront != []): 
        df_compra = df_compra.loc[df_compra['waterfront_is'].isin(f_waterfront)]
    elif (f_waterfront != []):
        df_compra = df_compra.loc[df_compra['waterfront_is'].isin(f_waterfront), :]
    else:
        df_compra = df_compra.copy() 
    ...
    'Quantidade de Quartos', df_compra['bedrooms'].unique() )

    if (f_bedrooms != []): 
        df_compra = df_compra.loc[df_compra['bedrooms'].isin(f_bedrooms)]
    elif (f_bedrooms != []):
        df_compra = df_compra.loc[df_compra['bedrooms'].isin(f_bedrooms), :]
    else:
        df_compra = df_compra.copy() 
 ...
 # filter
    ...
    C12.metric(label = 'Valor Custo Total', value = '$ {:,.0f}'.format(np.round(df_compra['price'].sum(),2) ))
 
    col1, col2 = st.columns(2)
    col1.metric(label = 'Valor Venda Total', value = '$ {:,.0f}'.format(np.round(df_venda['preco_venda'].sum(),0) ))
    col2.metric(label = 'Lucro Total', value = '$ {:,.0f}'.format(np.round(df_venda['lucro'].sum(),0) ))
